...
int main() {
  ...
  char st[17]="PLAYER 1\n\n BUTTS", st2[10]="YOU SUCK!", st3[6]="PAUSE", st4[9]="        ";
  unsigned int i, j, a, b=0, c, bx=5, by=11, x=0, y=0;
  signed int dx=0, dy=0, px=80, py=200, xdir[4]={-2,-1,1,2}, ydir[4]={-1,-2,-2,-1};
  ...
  setsprite(1, px, py, 15, 0x31);
  setsprite(2, px+8, py, 16, 0x31);
  setsprite(3, px+16, py, 16, 0x31+64);
  setsprite(4, px+24, py, 17, 0x31);

  // shadow sprites
  setsprite(5, x+3, y+3, 21, 0x11);
  setsprite(6, px+4, py+4, 18, 0x11);
  setsprite(7, px+12, py+4, 19, 0x11);
  setsprite(8, px+20, py+4, 19, 0x11+64);
  setsprite(9, px+28, py+4, 18, 0x11+64);
  delay(50); 
...
label1:
  ...
  if ((getjoystatus(0) & A_BUTTON) != 0) {
    ...
    if ((getjoystatus(0) & UP_BUTTON) != 0) py -= 4;
    if ((getjoystatus(0) & DOWN_BUTTON) != 0) py += 4;
  } else {
    if ((getjoystatus(0) & RIGHT_BUTTON) != 0) px += 2;
    if ((getjoystatus(0) & LEFT_BUTTON) != 0) px -= 2;
    if ((getjoystatus(0) & UP_BUTTON) != 0) py -= 2;
    if ((getjoystatus(0) & DOWN_BUTTON) != 0) py += 2;
  }
  ...
  if ((y < 15) && (y != 0)) {
    ...
    if ((y < py + 3) && (y >= py - 3)) {
      if ((x >= px) && (x <= px+27)) {
        ...
        lives--; x=0; y=0; px=80; py=200; dx=0; dy=0;
        writenum(lives, 8, blockmap, 0x136, 0x426);
        writestring(st, blockmap, 0x248, 0x3F6);
        setmap(0, (unsigned char*)blockmap);
    ...
    if ((b >= 0) && (b<100)) {
      if (blocks[b] != 8) {
        ...
        if (blockcount == 0) {
          // new level
          level++; level2++; x=0; y=0; px=80; py=200;
